...
#CLASS CAMB 7140
#DATE: Jan 22, 2025
#OBJECTIVE: bash basics, read mapping with kallisto
...
# Change permissions on your computer so that you can run a shell script by typing: 'chmod 777 readMapping.sh' (without the quotes) at the terminal prompt --> this gives all users read, write, and execute permissions, so a+rwx
...
#for zsh I need to include the "" in file names, as it requires string inputs for index --> run separately 
...
###from class notes...
exit 1
#search for file with fastq anywhere in the name *search_paramter*
for FASTQ in *fastq*
do
    #set output file name, search for fastq.gz, replace with mapped to determine output FOLDER name in callisto
